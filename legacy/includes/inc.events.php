...
$e = new cSystemEvent($config->legacy->MONTHLY_LISTING_UPDATES, MONTHLY*1440);
if ($config->legacy->EMAIL_LISTING_UPDATES and $e->TimeForEvent()) {
	$members = new cMemberGroup();
	$members->EmailListingUpdates(MONTHLY);
	$e->LogEvent();
}

$e = new cSystemEvent($config->legacy->WEEKLY_LISTING_UPDATES, WEEKLY*1440);
if ($config->legacy->EMAIL_LISTING_UPDATES and $e->TimeForEvent()) {
	$members = new cMemberGroup;
...
$e = new cSystemEvent($config->legacy->DAILY_LISTING_UPDATES, DAILY*1440);
if ($config->legacy->EMAIL_LISTING_UPDATES and $e->TimeForEvent()) {
	$members = new cMemberGroup;
