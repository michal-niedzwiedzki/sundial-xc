...
if ($_REQUEST["process"]==1) {
	...
	$output = cIncomeTies::saveTie(array("member_id"=>$user->member_id, "amount"=>$amount, "tie_id"=>$tie_id))."<p>";
	//$p->DisplayPage($output);
...
if ($_REQUEST["remove"]==1) {
	...
	$output = cIncomeTies::deleteTie($user->member_id)."<p>";
...
$myTie = $ties->getTie($user->member_id);
